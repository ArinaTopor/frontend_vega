...
import { ModalWorkerFile } from '../ModalWorkerFile/ModalWorkerFile';
...
export function ModalWorkerOrder({open, setOpen, name, setTable, index, table}:Props){
	...
	return(
		<>
				...
				<Flex vertical>
					...
					<Flex vertical gap='30px'className={styles.body}>
						...
						<Flex vertical gap='20px'>
							...
							<Flex className={`${styles.field} ${styles.file}`} onClick={()=>setOpenFile(true)}>
								qwertyu.pdf
							</Flex>
						</Flex>
						<Flex vertical gap='20px'>
			...
			<ModalWorkerFile open={openFile} setOpen={setOpenFile}/>
		</>
	)
}